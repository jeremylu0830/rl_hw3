...
# Add this function to your utils/envs.py file if it doesn't handle episodes correctly

def play_episode_rb(env, policy, buf):
    """
    Play an episode according to a given policy and add to a replay buffer.
    For recurrent buffers, transitions are collected and stored as complete episodes.
    ...
    while not done:
        ...
        # For recurrent buffer, it will accumulate until episode ends (done=True)
        # For regular buffer, it adds immediately
...
# If you also want to update play_episode to handle hidden states properly:
